...
#отримуєм ID модератора
#@dp.message_handler(commands=['moderator'], is_chat_admin=True)#, reply_markup=button_case_admin)
async def make_changes_command(message : types.Message):
    global ID
    ID = message.from_user.id
    await bot.send_message(message.from_user.id, 'Що, директоре, вам треба?', reply_markup=admin_kb.button_case_admin)
...
async def cancel_hander(message : types.Message, state : FSMContext):
    if message.from_user.id == ID:
...
async def load_description(message : types.Message, state : FSMContext):
    if message.from_user.id == ID:
        ...
        await FSMAdmin.next()
        await message.reply('Тепер вкажить ціну')
...
async def delete_item(message : types.Message):
    if message.from_user.id == ID:
        ...
        for ret in read:
            await bot.send_photo(message.from_user.id, ret[0], f'{ret[1]}\Опис: {ret[2]}\Ціна: {ret[-1]}')
...
def register_handlers_admin(dp : Dispatcher):
    ...
    dp.register_message_handler(cancel_hander, Text(equals='отмена', ignore_case=True), state="*")
    dp.register_message_handler(make_changes_command, commands=['moderator'],is_chat_admin=True)  # , reply_markup=button_case_admin))
    dp.register_message_handler(load_photo, content_types=['photo'], state=FSMAdmin.photo)
    dp.register_message_handler(load_name, state=FSMAdmin.name)
    dp.register_message_handler(load_description, state=FSMAdmin.description)
    dp.register_message_handler(load_price, state=FSMAdmin.price)
    dp.register_message_handler(cancel_hander, state="*", commands='отмена')
